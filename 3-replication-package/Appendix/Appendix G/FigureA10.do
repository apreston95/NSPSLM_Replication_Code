...
global shock bs_tfp_news
global trendlist t


foreach var in lrgdp ltfp_util lrealndcserv lunemployment lvacancies  ffr psr lrstockp pvs real_rate {
 quietly gen b`var' = .
...
tw (rarea up90`var' lo90`var' Quarters, bcolor(cranberry*.3) clw(medthin medthin) title(`: variable label `var'') legend(off))  (line b`var' Quarters, ysc(r(-1.5 0.5)) ylabel(-1.5[0.5]0.5) lw(medthick) lc(cranberry) ytitle("% Deviation") xtitle("Quarters")) (scatteri 0 0 0 20, recast(line) lp(longdash) lw(medthin) lc(cranberry)) if Quarters<=20 ,saving(`shock'_`var'_LP.gph,replace) 
}
...
graph export FigureA10.png, as(png) width(2000) replace
