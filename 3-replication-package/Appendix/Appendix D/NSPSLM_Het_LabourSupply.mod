...
Var W R Pi Eta b Y A N M Eta_V MC U UN V JC C C_UU C_E_0 B_E_0 Psi_E_0 Ave_Cons_Loss logY logA logEta SavingRate URisk h
...
// Employed BCs
C_E_0 + B_E_0 = W*h + Eps_MPC ;

@#for k in 1:KBAR-1

C_E_@{k} + B_E_@{k} = W*h + R(-1)/(1+Pi)*(B_E_@{k-1}(-1) + Eps_MPC) ;

@#endfor

@#for k in KBAR:KBAR

C_E_@{k} + B_E_@{k} = W*h + R(-1)/(1+Pi)*(B_E_@{k}(-1) + Eps_MPC) ;
...
(C_E_@{k})^(-Gamma) = Beta*(R/(1+Pi(+1)))*((1-Rho*(1-Eta(+1)))*(C_E_@{k+1}(+1) - (kappa_0/(1+kappa_1))*h^(1+kappa_1) )^(-Gamma) + Rho*(1-Eta(+1))*(C_EU_@{k+1}(+1))^(-Gamma));

@#endfor

@#for k in KBAR:KBAR

(C_E_@{k})^(-Gamma) = Beta*(R/(1+Pi(+1)))*((1-Rho*(1-Eta(+1)))*(C_E_@{k}(+1) - (kappa_0/(1+kappa_1))*h^(1+kappa_1))^(-Gamma) + Rho*(1-Eta(+1))*(C_EU_@{k}(+1))^(-Gamma));
...
[name='MC']
MC = (1/(A*h))*(W + (Kappa/Eta_V) - Beta*(1-Rho)*(Kappa/Eta_V(+1)));
