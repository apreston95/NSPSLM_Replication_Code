...
Var W R Pi Eta b Y A N M Eta_V MC U UN V JC C C_UU C_E_0 B_E_0 Psi_E_0 Ave_Cons_Loss logY logA logEta SavingRate URisk Rho_t 
...
(C_E_@{k})^(-Gamma) = Beta*(R/(1+Pi(+1)))*((1-Rho_t(+1)*(1-Eta(+1)))*(C_E_@{k+1}(+1))^(-Gamma) + Rho_t(+1)*(1-Eta(+1))*(C_EU_@{k+1}(+1))^(-Gamma));

@#endfor

@#for k in KBAR:KBAR

(C_E_@{k})^(-Gamma) = Beta*(R/(1+Pi(+1)))*((1-Rho_t(+1)*(1-Eta(+1)))*(C_E_@{k}(+1))^(-Gamma) + Rho_t(+1)*(1-Eta(+1))*(C_EU_@{k}(+1))^(-Gamma));
...
URisk = 1 - ((1-Rho_t(+1)*(1-Eta(+1)))*(1-Rho_t(+2)*(1-Eta(+2)))*(1-Rho_t(+3)*(1-Eta(+3)))*(1-Rho_t(+4)*(1-Eta(+4))));

JC = 1 - Rho_t(+1)*(1-Eta(+1));
...
[name='MC']
MC = (1/A)*(W + (Kappa/Eta_V) - Beta*(1-Rho_t(+1))*(Kappa/Eta_V(+1)));
...
[name = 'Job Loss Rate Function']
Rho_t/Rho = (MC/steady_state(MC))^(-iota) ;
...
IRF_JL_NEWS4 = oo_.irfs.Rho_t_Eps_A4;
...
subplot(3,3,9)
plot(0:H,IRF_JL_NEWS4(1:end)*100,'color','black','LineWidth',2);
hold on
line([0 H],[0 0],'color','black','LineStyle','-');
ylabel('Ppt Deviation','interpreter','LaTeX');
xlabel('Quarters','interpreter','LaTeX');
title('Job Separation Rate','interpreter','LaTeX');
