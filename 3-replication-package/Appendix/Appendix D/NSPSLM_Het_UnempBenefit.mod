...
Var W R Pi Eta tau Y A N M Eta_V MC U UN V JC C C_UU C_E_0 B_E_0 Psi_E_0 Ave_Cons_Loss logY logA logEta SavingRate URisk 
...
set_param_value('b_Bar',0.75*par.W_Bar);
...
// Employed BCs
C_E_0 + B_E_0 = W*(1-tau) + Eps_MPC ;

@#for k in 1:KBAR-1

C_E_@{k} + B_E_@{k} = W*(1-tau) + R(-1)/(1+Pi)*(B_E_@{k-1}(-1) + Eps_MPC) ;

@#endfor

@#for k in KBAR:KBAR

C_E_@{k} + B_E_@{k} = W*(1-tau) + R(-1)/(1+Pi)*(B_E_@{k}(-1) + Eps_MPC) ;
